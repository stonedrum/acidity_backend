...
async def init_db():
    async with engine.begin() as conn:
        ...
        try:
            ...
            # 新增区域相关字段
            await conn.execute(text("ALTER TABLE documents ADD COLUMN IF NOT EXISTS region_level VARCHAR"))
            await conn.execute(text("ALTER TABLE documents ADD COLUMN IF NOT EXISTS province VARCHAR"))
            await conn.execute(text("ALTER TABLE documents ADD COLUMN IF NOT EXISTS city VARCHAR"))
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 检查是否已有数据
        res = await session.execute(select(Region).limit(1))
        if not res.scalar():
            print("[init_db] 正在初始化区域数据...")
            regions_data = {
                ...
                "新疆维吾尔自治区": ["乌鲁木齐市", "克拉玛依市", "吐鲁番市", "哈密市", "昌吉回族自治州", "博尔塔拉蒙古自治州", "巴音郭楞蒙古自治州", "阿克苏地区", "克孜勒苏柯尔克孜自治州", "喀什地区", "和田地区", "伊犁哈萨克自治州", "塔城地区", "阿勒泰地区", "自治区直辖县级行政单位"]
            }
            
            province_count = 0
            city_count = 0
            for province_name, cities in regions_data.items():
                province = Region(name=province_name, level=1)
                session.add(province)
                await session.flush()
                province_count += 1
                for city_name in cities:
                    city = Region(name=city_name, level=2, parent_id=province.id)
                    session.add(city)
                    city_count += 1
            await session.commit()
            print(f"[init_db] 区域数据初始化完成: {province_count} 个省/直辖市, {city_count} 个市")
